...
def train(starting_epoch, model, optimizer, scheduler, trainer, evaluator, ENV):
    for epoch in range(starting_epoch, config.epochs):
        ...
        ENV['global_step'] = trainer.train(epoch, model, optimizer, 
                                        args.robust_noise, args.robust_noise_step, args.avgtimes, 
                                        random_noise=None, pue=args.pue)
    ...
    while condition:
        ...
        if args.attack_type == 'min-min' and not args.load_model:
            ...
            for j in pbar_train:
                ...
                if (args.pue or args.pueb) and switch:
                    trainer.train_batch_robust(torch.stack(train_imgs).to(device), labels, model, optimizer, 
                                               args.robust_noise, args.robust_noise_step, args.avgtimes, pbar_train)
                else:
                    trainer.train_batch(torch.stack(train_imgs).to(device), labels, model, optimizer)

        pbar_perturb = tqdm(enumerate(data_loader[args.universal_train_target]), total=len(data_loader[args.universal_train_target]))
        for i, (images, labels) in pbar_perturb:
            ...
            if args.attack_type == 'min-min':
                if switch and args.pue:
                    perturb_img, eta = noise_generator.min_min_attack_robust(images, labels, model, optimizer, criterion, 
                                                                    args.robust_noise, args.robust_noise_step, args.avgtimes_perturb,
                                                                    pbar_train, random_noise=batch_noise)
                else:
...
def sample_wise_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV):
    ...
    while condition:
        ...
        if args.attack_type == 'min-min' and not args.load_model:
            # Train Batch for min-min noise
            pbar_train = tqdm(range(0, args.train_step), total=args.train_step)
            for j in pbar_train:
                try:
                    (images, labels) = next(data_iter)
                except:
                ...
                for param in model.parameters():
                    param.requires_grad = True
                if (args.pue or args.pueb) and switch:
                    trainer.train_batch_robust(images, labels, model, optimizer, 
                                               args.robust_noise, args.robust_noise_step, args.avgtimes, pbar_train)
                else:
        ...
        # Search For Noise
        idx = 0
        pbar_perturb = tqdm(enumerate(data_loader['train_dataset']), total=len(data_loader['train_dataset']))
        for i, (images, labels) in pbar_perturb:
            images, labels, model = images.to(device), labels.to(device), model.to(device)
...
def main():
    ...
    if args.attack_type == 'random':
        ...
        util.save_model(ENV=ENV,
                        ...
                        is_best=False,
                        filename=checkpoint_path_file)
        logger.info('Surrogate Model Saved at %s', checkpoint_path_file)
    else:
