...
parser.add_argument('--version', type=str, default="resnet18_recover")
parser.add_argument('--model_version', type=str, default="resnet18")
...
parser.add_argument('--recover_rate', default=0.2, type=float, help='proportion of training data used for recovery attack')
# Training Options
...
checkpoint_path = os.path.join(args.surrogate_path, args.model_version)
checkpoint_path = os.path.join(checkpoint_path, 'checkpoints')
checkpoint_path_file = os.path.join(checkpoint_path, args.model_version)
...
def train(starting_epoch, model, poison_model, optimizer, scheduler, trainer, evaluator, ENV, eta):
    ...
    for epoch in range(starting_epoch, config.epochs):
        ...
        # Train
        ENV['global_step'] = trainer.train_recovery(epoch, model, optimizer, poison_model, eta, project=args.project)
...
def main():
    ...
    if args.use_train_subset:
        ...
        data_loader = datasets_generator._split_validation_set(train_portion=args.recover_rate, train_shuffle=True, train_drop_last=True)
    else:
    ...
    if args.load_model:
        ...
        # This case is not useful for the paper's experiment, as a target model is required.
        logger.warning("Warning: --load_model is not set. The target model will be random.")
        poison_model = copy.deepcopy(model)

    # You can optionally add a log to confirm the starting accuracies
    evaluator.eval(0, model)
    logger.info('Starting accuracy of RECOVERY model (from scratch): %.2f' % (evaluator.acc_meters.avg*100))
    evaluator.eval(0, poison_model)
    logger.info('Accuracy of TARGET model (loaded): %.2f' % (evaluator.acc_meters.avg*100))
